            ...
            success: function (response) {
                if (response.status === "success") {
                    ...
                    $("#notificationsDropdown").fadeOut(500, function () {
                        $(this)
                            .html(
                                '<li><p class="no-notifications">No notifications</p></li>'
                            )
                            .fadeIn(600);
                    });
                }
        ...
        success: function (response) {
            ...
            if (response.status === "success") {
                ...
                $.each(convo_rooms, function (i, convo_room) {
                    ...
                    var isConversationBlocked = convo_room.is_blocked === "true";
                    var blockUnblockBtn = "";
...
// Get messages
function getMessages(currentUserID, user_receiver_info) {
    var messagesHTML = "";

    // Loop through the get_messages array and generate the HTML for each non-empty message
    for (var i = 0; i < user_receiver_info.get_messages.length; i++) {
        var message = user_receiver_info.get_messages[i];
        var isSender = message.sender_id === currentUserID;
        var messageType = isSender ? "sender" : "receiver";

        // Check if the message is not blank (non-empty)
                ...
                messagesHTML += `
					<div class="message-wrapper">
						<div class="message sender">
							...
							<img class="avatar" src="src/css/images/free-display-photos/${message.sender_user_avatar}" alt="${message.sender_user_avatar}">
						</div>
					</div>
				`;
            } else {
                messagesHTML += `
					<div class="message-wrapper">
						<div class="message receiver">
							<img class="avatar" src="src/css/images/free-display-photos/${message.receiver_user_avatar}" alt="${message.receiver_user_avatar}">
							<div class="message-content">
								<p>${message.messages}</p>
							</div>
						</div>
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === "success") {
                    ...
                    var newHTML = `
                        <p class="text-danger mb-0 fw-light">By blocking this user, you've prevented them from sending you messages.</p>
                    `;
