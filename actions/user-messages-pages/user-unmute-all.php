    ...
    if ($current_user_id === $user_id) {
        ...
        if ($get_convo_result && mysqli_num_rows($get_convo_result) > 0) {
            ...
            if ($conversation_data['sender_id'] == $user_id) {
                $update_query = "UPDATE `user_conversation_room` SET `sender_muted`='false' WHERE sender_id = '$user_id'";
                $update_result = mysqli_query($con, $update_query);

                $data['status'] = "success";
                $data['message'] = "Current user is the sender.";
            } else {
                $update_query = "UPDATE `user_conversation_room` SET `receiver_muted`='false' WHERE receiver_id = '$user_id'";
                $update_result = mysqli_query($con, $update_query);
