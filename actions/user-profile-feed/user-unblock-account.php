...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($logged_in_user_id === $current_user_id) {
        ...
        if ($get_users_result && mysqli_num_rows($get_users_result) > 0) {
            $query = "DELETE FROM `blocked_users` WHERE blocking_user_id = '$current_user_id' AND blocked_user_id = '$reported_user_id'";
            $result = mysqli_query($con, $query);

            if ($result) {
                $update_query = "UPDATE `user_conversation_room` SET `status`='$user_status', `is_blocked`='false' WHERE convo_room_id = '$convo_room_id'";
